...
def run_vectorized(params: dict, config: SimConfig) -> dict[str, Any]:
    ...
    burn_rate = sample_burn_rate_g_s(rng, n)
    yield_factor = sample_yield_factor(rng, n)
    raw_duration = raw_burn_duration_s(g["filler_mass_g"], burn_rate)
    ...
    depth = rng.uniform(2.0, 4.0, size=n)

    aerosol = aerosol_mass_g(g["filler_mass_g"], burn_rate, yield_factor, config.n_grenades)
    ...
    moe_mask = moe_fused_eoir_mask(t_vis, t_mwir, moe["transmittance_threshold"], emp["visual_smoke_factor"])

    build_up = build_up_time_s(rng, n, burn_rate, cloud["build_up_time_s"]["min"], cloud["build_up_time_s"]["max"])
    duration = effective_duration_s(raw_duration, wind_mph, rng, n)
    lock_break = lock_break_duration_estimate_s(duration, moe_mask, np.zeros(n), moe["lock_break_duration_threshold_s"])
    ...
    return {
        ...
        "kpp_checks": {
            "build_up_p90_le_15s": pct(build_up, 90) <= 15.0,
            "duration_p10_ge_120s": pct(duration, 10) >= 120.0,
            "area_p10_ge_30_sqft": pct(area_sqft, 10) >= 30.0,
            "moe_lock_break_p50_ge_60s": pct(lock_break, 50) >= 60.0,
        },
