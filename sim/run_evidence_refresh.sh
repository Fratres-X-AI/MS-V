#!/usr/bin/env bash
# Refresh analysis reports from committed mega manifest (RunPod optional)
set -euo pipefail
cd "$(dirname "$0")/.."
export RUNPOD_CPU_COUNT="${RUNPOD_CPU_COUNT:-32}"
WORKERS="${WORKERS:-31}"

echo "=== MS-V evidence refresh $(date -u +%Y-%m-%dT%H:%M:%SZ) workers=$WORKERS ==="
...
Path("analysis/results/evidence_refresh.json").write_text(json.dumps({
...
print("Wrote analysis/results/evidence_refresh.json")
PY

echo "=== EVIDENCE REFRESH COMPLETE ==="
